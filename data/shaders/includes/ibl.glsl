...
vec3 getIBLRadianceGGX(samplerCube GGXEnvSampler, sampler2D GGXLUT, float mipCount, vec3 n, vec3 v, float perceptualRoughness, vec3 specularColor)
{
    float NdotV = clampedDot(n, v);
    float lod = clamp(perceptualRoughness * mipCount, 0.0, mipCount);
    vec3 reflection = normalize(reflect(-v, n));

    vec2 brdfSamplePoint = clamp(vec2(NdotV, perceptualRoughness), vec2(0.0, 0.0), vec2(1.0, 1.0));
    vec2 brdf = texture(GGXLUT, brdfSamplePoint).rg;
    vec4 specularSample = textureLod(GGXEnvSampler, reflection, lod);
...
vec3 getIBLRadianceLambertian(samplerCube lambertianEnvSampler, vec3 n, vec3 diffuseColor)
{
    vec3 diffuseLight = texture(lambertianEnvSampler, n).rgb;
